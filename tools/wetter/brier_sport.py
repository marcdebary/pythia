...
def zerlegung(pp, kuebel=5):
    """Murphy: Zuverlaessigkeit, Aufloesung, Ungewissheit."""
    n = len(pp)
...
for name, pp in (("wir", unser), ("Kalshi", markt)):
    zuv, res, ung = zerlegung(pp)
    print(f"   {name:8}  Zuverlaessigkeit {zuv:7.4f} (klein ist gut)"
          f"   Aufloesung {res:7.4f} (gross ist gut)"
          f"   Ungewissheit {ung:7.4f}")
    print(f"   {'':8}  Summe {zuv - res + ung:7.4f}  gegen gemessen {brier(pp):7.4f}"
          f"   {'stimmt' if abs(zuv - res + ung - brier(pp)) < 1e-9 else 'ABWEICHUNG'}")

zuv, res, ung = zerlegung(unser)
...
print(f"\n   n = {len(paare)}. Das ist viel zu wenig fuer eine Aussage.")
print(f"   Fuer einen Unterschied von 0,01 bei dieser Streuung ({sd:.3f}) braucht es")
print(f"   rund {int((1.96*sd/0.01)**2):,} Spiele.")
